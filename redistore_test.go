...
func NewRedisRecorder() *ResponseRecorder {
...
//const DefaultRedisRemoteAddr = "1.2.3.4"
...
func TestRediStore(t *testing.T) {
	...
	// RedisStore
	store, err := NewRediStore(10, "tcp", redisAddr, "", []byte("secret-key"))
	if err != nil {
		t.Fatal(err.Error())
	}
	defer store.Close()
	...
	// RedisStoreWithDB
	store, err = NewRediStoreWithDB(10, "tcp", redisAddr, "", "1", []byte("secret-key"))
	if err != nil {
		t.Fatal(err.Error())
	}
	defer store.Close()
...
func TestPingGoodPort(t *testing.T) {
	store, _ := NewRediStore(10, "tcp", redisAddr, "", []byte("secret-key"))
	defer store.Close()
	ok, err := store.ping()
	if err != nil {
		t.Error(err.Error())
